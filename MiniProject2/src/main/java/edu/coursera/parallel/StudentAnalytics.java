...
public final class StudentAnalytics {
    ...
    /**
     * COMPLETE: compute the average age of all actively enrolled students using
    ...
    /**
     * COMPLETE compute the most common first name out of all students that are no
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(final Student[] studentArray) {
        return (int) Arrays.stream(studentArray)
                .parallel()
                .filter(s -> s.getAge() > 20.0 && s.getGrade() < 65 && !s.checkIsCurrent())
                .count();
    }
}
